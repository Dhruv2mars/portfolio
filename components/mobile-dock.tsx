 ...
 * is drawn above it.
 *
 * It floats `--signature-clear` above the bottom rather than on it, because
 * the footer signature runs off that edge and a dock sitting there would cover
 * the whole word. That distance tracks the viewport width, since the mark does.
 */
export function MobileDock({ items }: { items: readonly NavItem[] }) {
  return (
    <div className="fixed bottom-[calc(var(--signature-clear)+env(safe-area-inset-bottom,0px))] left-1/2 z-[60] flex w-fit -translate-x-1/2 items-center rounded-xl bg-popover py-1 pr-1 pl-1.5 ring ring-foreground/10 sm:hidden dark:ring-foreground/20">
      <PaletteTrigger dock />
