...
**Editorial surface**:
The Portfolio’s visual and UX north star — warm monochrome editorial craft in the lineage of pablostanley.com / loganliffick.com / evilrabbit.com: warm-tinted neutrals (never cool gray), hairline-divided CV rows (name → one-line description → url ↗ → year/date), 12px uppercase micro-labels with wide tracking, Geist type and spacing doing the work. Display titles (name, page H1s) are set in IBM Plex Serif — the editorial voice; everything else stays Geist. No chromatic accent in chrome; blue survives only inside the AI Activity heatmap. Flat background — no gradients, no noise. Light is the primary theme; a polished warm dark theme is also available. All text meets WCAG AA in both themes (faint meta ≥ 4.5:1). Motion stays quiet and intentional (theme transition, hovers, floating heatmap tooltip, staggered fade-rise entrances) — craft through restraint, not spectacle. Signature details: inverted-ink text selection, a live local-time readout (Kolkata, IST) in the footer, and a ⌘K command menu that goes anywhere from anywhere.
...
**Command menu**:
A global ⌘K / Ctrl+K overlay (also a header ⌘K chip) listing navigation, actions, and social links — filter as you type, arrows cycle, Enter runs, Esc closes. Actions include copy email address, theme switch, and view source. Small type, hairline shell, no logo chrome. It is the fast path; the three-item Primary nav stays the visible IA.
_Avoid_: Palette replacing the nav, nested submenus, fuzzy ranking, hero-toy treatment

**AI Activity**:
A GitHub-contribution-style heatmap of Dhruv’s AI usage on Home under the profile/intro. Year grid of day cells (intensity by tokens), floating inverted-ink tooltip on hover/focus (tokens + date), live-today cell with a soft ring pulse (synthetic projection, eased count-up), quiet legend, plus a single lifetime-tokens summary. Nightly ingest publishes complete days to Vercel Blob; today is projected client-side. No Daily/Weekly/Cumulative tabs or streak/analytics chrome.
_Avoid_: Analytics dashboard, Observability, “stats section”, copying a specific product’s chrome

**Home**:
The landing surface. One centered 42rem column for everything — header, sections, footer. Section order: profile/intro → AI Activity → selected Projects → latest Writings (only if at least one Post is published) → footer. Primary nav destination. The intro is a short rewritten editorial block (name + brief positioning + pulsing status line — currently “Open to interesting problems.” — + mono socials row) — not the old bio, and not omitted. The Email social copies the address on click (label swaps to “Copied”) instead of launching a mail client. Sections separate by whitespace (~64px) only; hairlines belong to lists, not section boundaries. Footer is minimal (name/©, city, live local time, mono socials + RSS); no mega-footer.
